...
{
    internal class AlriightyDebuggableProjectCfg : IVsDebuggableProjectCfg, IVsProjectFlavorCfg
    {
        private IVsProjectFlavorCfg _baseProjectCfg;
        private readonly EnvDTE.Project _baseProject;

        public AlriightyDebuggableProjectCfg(IVsProjectFlavorCfg baseProjectCfg, EnvDTE.Project project)
        {
        ...
        public int DebugLaunch(uint grfLaunch)
        {
            ...
            var startInfo = new AlriightyStartInfo(startArgs, null, _baseProject, AlriightySessionType.AttachEditorDebugger) { WorkingDirectory = AlriightyToolsPackage.Instance.SolutionEventsListener?.SolutionDirectory };
            ...
            var session = new AlriightyDebuggerSession();
            ...
            var startArgs = new SoftDebuggerConnectArgs(_baseProject.Name, IPAddress.Parse("127.0.0.1"), AlriightyToolsPackage.Instance.GeneralOptions.ConnectionPort)
            {
                MaxConnectionAttempts = AlriightyToolsPackage.Instance.GeneralOptions.MaxConnectionAttempts
            };

            var startInfo = new AlriightyStartInfo(startArgs, null, _baseProject, AlriightySessionType.AttachEditorDebugger)
            {
                WorkingDirectory = AlriightyToolsPackage.Instance.SolutionEventsListener?.SolutionDirectory
            };
        ...
        public int get_CfgType(ref Guid iidCfg, out IntPtr ppCfg)
        {
            ...
            try
            {
                ...
                {
                    ppCfg = Marshal.GetComInterfaceForObject(AlriightyDebugTargetSelection.Instance,
